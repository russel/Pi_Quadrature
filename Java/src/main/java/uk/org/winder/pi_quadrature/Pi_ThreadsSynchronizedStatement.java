...
public class Pi_ThreadsSynchronizedStatement {
  //  A singleton constant object used for all the synchronization.  As pointed out by Roger Orr (private
  //  communication, 2010-04-19) there is a subtle bug in using the sum variable (if it were object) to
  //  synchronize actions on itself since the variable refers to a different object after the update
  //  operation compared to the one it refers to when the object lock is claimed.
  ...
  private static void execute(final int numberOfTasks) {
    ...
    Output.out(Pi_ThreadsSynchronizedStatement.class, pi, n, elapseTime, numberOfTasks);
  }

  public static void main(final String[] args) {
    Pi_ThreadsSynchronizedStatement.execute(1);
    Pi_ThreadsSynchronizedStatement.execute(2);
    Pi_ThreadsSynchronizedStatement.execute(8);
    Pi_ThreadsSynchronizedStatement.execute(32);
  }
}
