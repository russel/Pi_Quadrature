...
javaEnvironment = Environment (
    tools = [ 'javac' ] ,
    JAVACFLAGS = [ '-source' , '7' , '-encoding' , 'utf-8' ] ,
    ENV = os.environ ,
    )
...
for item in Glob ( 'Pi_Java_*.java' ) :
    ...
    javaCommand = 'java'
    compiledBits = [ javaEnvironment.Java ( target = '.' , source = 'JOutput.java' ) ]
    if variant in [ 'DataRush4' , 'DataRush5' ] :
        dataRushHome = os.environ['HOME'] + '/lib/Java/' + variant
        compiledBits.append ( javaEnvironment.Java ( target = '.' , source = item , JAVACLASSPATH = [ f.path for f in Glob ( dataRushHome + '/lib/*.jar' ) ] ) )
        javaCommand = 'JAVA_HOME=' + os.environ['JAVA_HOME'] + ' ' + dataRushHome + '/bin/dr -cp . '
    elif variant in classpathEntries.keys ( ) :
        compiledBits.append ( javaEnvironment.Java ( target = '.' , source = item , JAVACLASSPATH = classpathEntries[variant] ) )
        javaCommand += ' -cp .:' + ':'.join ( classpathEntries[variant] )
    else :
        compiledBits.append ( javaEnvironment.Java ( target = '.' , source = item ) )
