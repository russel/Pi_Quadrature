...
class partialSum {
 ...
 public:
  ...
  void operator()(tbb::blocked_range<long> const & range) {
		// TBB does not support range-based for :-(
    //for (auto i = range.begin(); i != std::end(range); ++i) {
...
int main() {
  ...
  out("TBB Batched", pi, n, elapseTime, tbb::task_scheduler_init::default_num_threads(), tbb::task_scheduler_init::default_num_threads());
  return 0;
}
