...
/*
 *  Caluclation of π using quadrature realized with GPars dataflow tasks and a dataflow queue.
 *
...
void execute(final operatorCount) {
  ...
  (0 ..< operatorCount).each {index ->
    task {partialSums << PartialSum.dynamicCompile(index, sliceSize, delta)}
  }
